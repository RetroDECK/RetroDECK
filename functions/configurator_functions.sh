...
find_empty_rom_folders() {
  # This function will build an array of all the system subfolders in $rd_home_roms_path which are either empty or contain only systeminfo.txt for easy removal
  ...
  for system in $(find "$rd_home_roms_path" -mindepth 1 -maxdepth 1 -type d -printf '%f\n')
  do
